...
type PetStoreLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.PetStore, err error)
...
// List lists all PetStores in the indexer.
func (s *petStoreLister) List(selector labels.Selector) (ret []*v1alpha1.PetStore, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.PetStore))
	})
	return ret, err
}
...
type PetStoreNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.PetStore, error)
...
// List lists all PetStores in the indexer for a given namespace.
func (s petStoreNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.PetStore, err error) {
...
// Get retrieves the PetStore from the indexer for a given namespace and name.
func (s petStoreNamespaceLister) Get(name string) (*v1alpha1.PetStore, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("petstore"), name)
	}
	return obj.(*v1alpha1.PetStore), nil
}
